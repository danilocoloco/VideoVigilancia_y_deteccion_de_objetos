...
def marcar_zonas(frame_mov, frame_original,out):
  ...
  im,contours, hierarchy = cv2.findContours(limites.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
  ...
  if movimiento_detectado:
    ...
    cv2.putText(frame_original, ".REC", (5,230), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255,255,255))
    #grabamos el fichero con el movimiento detectado
    cv2.imwrite("captura_actual.jpg", frame_original)
    timestamp = datetime.datetime.now()
    ts = timestamp.strftime("%d %B %Y %I:%M:%S%p")
    cv2.rectangle(frame_original, (2, 220), (185, 235), (0, 0, 0), -1)
    cv2.putText(frame_original, ts, (5, 230), cv2.FONT_ITALIC, 0.35, (255, 111, 255))
...
original = cv2.resize(cam.read()[1],(720,576), interpolation = cv2.INTER_CUBIC)

fourcc = cv2.VideoWriter_fourcc(*'XVID')
out = cv2.VideoWriter("ABRIR/" + time.strftime("%d_%m_%y_%H%M%S") + '.avi', fourcc, 20.0, (640, 480))

while True:
  # Calculamos la zonas que se han modificado
  imagen_delta = diffImg(cv2.resize(t_minus,(720,576), interpolation = cv2.INTER_CUBIC), cv2.resize(t,(720,576), interpolation = cv2.INTER_CUBIC), cv2.resize(t_plus,(720,576), interpolation = cv2.INTER_CUBIC))
  imagen_zonas_marcadas = marcar_zonas(imagen_delta, original.copy(),out)
