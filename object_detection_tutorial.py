...
TEST_IMAGE_PATHS = [ os.path.join(PATH_TO_TEST_IMAGES_DIR, 'image{}.jpg'.format(i)) for i in range(6, 25) ]
  ...
  with tf.Session(graph=detection_graph) as sess:
    ...
    for image_path in TEST_IMAGE_PATHS:
      ...
      for inc in range(len(indices)):
          if(np.squeeze(scores)[inc]>0.6):
              print("Hay un",#np.squeeze(scores)[inc],
               category_index[n[inc]]['name'])
          else:
      ...
      cv2.imshow('image', cv2.resize(image_np, (1920, 1080)))
